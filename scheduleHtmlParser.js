...
function scheduleHtmlParser(html) {
    ...
    // 找到课表所在位置
    let $raw = $('tr[style]').toArray() let trList = Array.from($raw)
    ...
    for (let tr of trList) {
        ...
        for (let courseInfoRaw of courseInfoRawArray) {
            ...
            // 处理上课时间
            let timeText = splited[0] let weekText = substringBefore(timeText, "星期") let weekDayAndSectionText = substringAfter(timeText, "星期") let week = substringBefore(substringAfter(weekText, "["), "周").replace("双", "").replace("单", "")

            // 星期
            let courseDay = week2Day(weekDayAndSectionText.substring(0, 1)) let section = substringBefore(substringAfter(weekDayAndSectionText, "["), "节")
            // 周
    ...
    // 判断月份 使用对应作息时间
    let sectionTime = sectionTimeSummer let today = new Date();
